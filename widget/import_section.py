...
class ImportSection:
    def __init__(self, master):
        ...
        self.dataloader_file_label = ctk.CTkLabel(self.file_frame, text="No DataSet selected", font=st.TEXT_FONT)
        self.dataloader_file_label.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        self.dataloader_remove_button = ctk.CTkButton(self.file_frame, text="X", command=self.remove_dataset_file, width=10, height=10, corner_radius=10)
        self.dataloader_remove_button.grid(row=1, column=2, padx=(0, 10), pady=5, sticky="e")
        
        self.dataset_file_label = ctk.CTkLabel(self.file_frame, text="No Data File selected", font=st.TEXT_FONT)
        self.dataset_file_label.grid(row=2, column=0, columnspan=2, padx=10, pady=5, sticky="w")

        self.dataset_remove_button = ctk.CTkButton(self.file_frame, text="X", command=self.remove_data_file, width=10, height=10, corner_radius=10)
    ...
    def import_dataset(self):
        ...
        if filename:
            ...
            self.dataloader_file_label.configure(text=f"DataSet: {os.path.basename(filename)}", font=st.TEXT_FONT)
    ...
    def import_datafile(self):
        ...
        if filename:
            ...
            self.dataset_file_label.configure(text=f"Data File: {os.path.basename(filename)}", font=st.TEXT_FONT)
    ...
    def remove_dataset_file(self):
        self.dataloader_file_path = None
        self.dataloader_file_label.configure(text="No DataSet selected", font=st.TEXT_FONT)
        
    def remove_data_file(self):
        self.dataset_file_path = None
        self.dataset_file_label.configure(text="No Data File selected", font=st.TEXT_FONT)
